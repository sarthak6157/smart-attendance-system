"""
seed.py - Seeds the database with admin and students from Excel files.
Runs on every server startup via main.py - safe even if files are missing.
"""
...
def main():
    ...
    try:
        ...
        if not existing_admin:
            ...
            db.add(admin)
            db.commit()
            print("Admin created. Login: admin1 / Pass@123")
        else:
    ...
    if not os.path.exists(file1) or not os.path.exists(file2):
        print("WARNING: Excel files not found. Skipping student import.")
        print("========== SEED COMPLETE ==========\n")
        db.close()
        return

    try:
        print("Found Excel files. Merging and importing...")
        ...
        for _, row in df.iterrows():
            try:
                ...
                db.add(User(
                    full_name=name, inst_id=enrollment_no, email=email,
                    role=UserRole.student, status=UserStatus.facial_required,
                    hashed_password=hash_password(raw_password), department=department,
                ))
                added_count += 1
    ...
    db.close()
    print(f"Students added: {added_count} | Skipped: {skipped_count}")
    print("========== SEED COMPLETE ==========\n")
