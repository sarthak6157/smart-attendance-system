...
from schemas.schemas import LoginRequest, PasswordChangeRequest, PasswordResetRequest, TokenResponse, UserCreate, UserOut
...
def manual_db_setup(db: Session = Depends(get_db)):
    try:
        ...
        db.add(admin)
        db.commit()
        return {"status": "success", "message": "Admin created. Login with admin1 / Pass@123"}
