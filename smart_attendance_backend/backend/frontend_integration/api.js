 ...
 * ========================================================
 * Drop this file in the /frontend folder alongside the HTML pages.
 * Every page that needs live data should include:
 *   <script src="api.js"></script>
 *
 * The module is exposed as a global `API` object.
 */

const API = (() => {
  const BASE_URL = "http://localhost:8000/api"; // Change to your server in production

  // -------------------------------------------------------------------------
  // Token helpers (stored in sessionStorage so it clears on tab close)
  // -------------------------------------------------------------------------
  ...
  const auth = {
     ...
     * Returns { access_token, user } and redirects automatically.
     */
    ...
    /** Returns the stored user without a network call. */
    currentUser() {
      return getUser();
    },

    /**
     * Guard helper – call at the top of each protected page.
     * Redirects to login if no token, or to the correct dashboard
     * if the user's role doesn't match the allowed list.
     */
    guard(allowedRoles = []) {
      const user = getUser();
      if (!user || !getToken()) {
      ...
      // Redirect pending / facial_required students
  ...
  const attendance = {
    ...
    markByFacial(sessionId) {
      return request("/attendance/facial", {
        method: "POST",
        body: {},
      });
      // Note: sessionId is passed as query param
  ...
  const biometrics = {
    /**
     * Enroll a face from a <canvas> or <img> element.
     * @param {number} userId
     * @param {HTMLCanvasElement|string} source – canvas element or base64 string
     */
  ...
  const reports = {
    ...
    exportCSV(courseId) {
      ...
      // Can't set auth header on anchor – open in new tab (server must allow)
  ...
  // -------------------------------------------------------------------------
  // Utility: QR code URL builder (uses free goQR service already in frontend)
  // -------------------------------------------------------------------------
